// [MERGE] lib/core/models/insight_models.dart (Stand: 2025-11-09, v6.4.0)
// Insight-Modelle (Facet, MoodPair, InsightScore, InsightFact, FactType)
// -----------------------------------------------------------------------------
...
// • V2-Map (snake_case) Ausgabe; rückwärtskompatibel für bestehende Call-Sites.
// • Neu/Kompat: typedef MemoryFact = InsightFact; enum FactType (für Imports
//   aus MemoryService u.a.); Tag-Normalisierung & robuste Zeitstempel-Parser.
// -----------------------------------------------------------------------------

import 'package:flutter/foundation.dart';

/// Kategorisierung für (Memory-)Fakten.
/// Hält Call-Sites kompakt; unverbindlich – UI/Services können eigene Mappings nutzen.
enum FactType { insight, identity, mood, timeline, recall, other }

/// Kompatibilität: Manche Call-Sites importieren `MemoryFact`.
/// In diesem Modellpaket entspricht das einer `InsightFact`.
typedef MemoryFact = InsightFact;
...
class InsightFact {
  ...
  factory InsightFact.fromWorker(dynamic raw) {
    ...
    List<String> pickTags() =>
        _coerceTags(m['tags'] ?? m['labels'] ?? m['keywords'] ?? m['facets']);
  ...
  static List<String> _coerceTags(dynamic raw) {
    ...
    void addOne(dynamic x) {
      ...
      if (x is String) {
        ...
        final parts =
            x.split(RegExp(r'[,\;]')).map((e) => e.trim()).where((e) => e.isNotEmpty);
        out.addAll(parts);
        return;
      }
      if (x is Map) {
        ...
        final label =
            (m['label'] ?? m['name'] ?? m['title'] ?? m['key'] ?? '').toString().trim();
        if (label.isNotEmpty) {
          out.add(label);
          return;
        }
    ...
    // Normalisieren & deduplizieren (klein, einzigartig)
    final set = <String>{};
    for (final t in out) {
