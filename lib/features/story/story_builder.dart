// [PATCHED] lib/features/story/story_builder.dart (Stand: 2025-11-08, v6.7.3)
// Kutsche 6 — StoryBuilder
// MERGE SIGNAL v6.7.3:
// • FIX: StoryResult benötigt jetzt ein required `id` → wird generiert.
// • Clean: unbenutzten Import entfernt.
// • Mood-Normierung / Topics-Handling wie v6.7.2.
//
// Erwartete Memory-Form (tolerant):
// context.memories: {
//   identity: { name: "…" },
//   mood: {
//     last: { ts: "...ISO...", mental: <num|string>, physical: <num|string> },
//     trend: { dir: "up|down|steady", delta: <num?> }
//   },
//   timeline: { items: [ { date:"ISO", topic:"…", topics:["…","…"], mood_delta:<num?> }, ... ] },
//   insights: [ { topic:"…", fact:"…" }, ... ],
//   recall: { weekly:"…", summary:"…" }
// }
//
// Ausgabe: StoryResult (services/guidance/dtos.dart) — { id, title, body, audioUrl? }
...
class StoryBuilder {
  ...
  static StoryResult _composeStory(
    ...
    // NEU: required id setzen (simple timestamp-basierte ID).
...
class StoryIngredients {
  ...
  factory StoryIngredients.fromMemories(Map<String, dynamic> m) {
    ...
    final last = (lastMental != null || lastPhysical != null)
        ? MoodPoint(
            mental: _normalizeMood(lastMental),
            physical: _normalizeMood(lastPhysical),
          )
        : null;
    ...
    // Recall
    final recall =
        _readStr(m, ['recall', 'weekly']) ?? _readStr(m, ['recall', 'summary']);
  ...
  static List<String> _collectTopicsFromTimeline(List<dynamic>? items) {
    ...
    for (final it in items) {
      if (it is Map<String, dynamic>) {
        ...
        final compact = _compactTopic(it);
        if (compact.isEmpty) continue;
        counts[compact] = (counts[compact] ?? 0) + 1;
      }
    }
  ...
  static List<Insight> _collectInsights(List<dynamic>? raw) {
    ...
    for (final it in raw) {
      if (it is Map<String, dynamic>) {
        ...
        final fact = _readStr(it, ['fact']) ??
            _readStr(it, ['text']) ??
            _readStr(it, ['note']);
        if ((topic != null && topic.trim().isNotEmpty) ||
            (fact != null && fact.trim().isNotEmpty)) {
  ...
  static String _compactTopic(String? s) {
    ...
    final x = s
        ...
        .trim();
    // Kürzen auf 3 Wörter
    final parts = x.split(' ').where((w) => w.isNotEmpty).toList();
  ...
  static double? _normalizeMood(num? v) {
    if (v == null) return null;
    // Akzeptiere Skalen: [0..1], [-1..1], [0..100] → normiere auf [-1..1]
    final d = v.toDouble();
    if (d >= 0.0 && d <= 1.0) return (d * 2.0) - 1.0; // 0..1 -> -1..1
    if (d >= -1.0 && d <= 1.0) return d; // schon normiert
    if (d >= 0.0 && d <= 100.0) return (d / 50.0) - 1.0; // 0..100 -> -1..1
