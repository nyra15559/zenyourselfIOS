...
//
// JournalEntry Model — Oxford-Zen v3.2 (null-safe, tolerant, legacy-aware, UI-ready)
// --------------------------------------------------------------------
// • Einheitliches Modell: reflection | journal | story
// • fromJson()/fromMap(): robust (String **oder** Map; akzeptiert Legacy-Keys)
// • Legacy-Shims: JournalType + .type, .text, .moodLabel (Tag-Parsing)
// • Legacy-Mapping u. a.:
//   - type/kind:  "note"→journal, "short_story"→story, bool isReflection→reflection
//   - Felder:     text→thoughtText, label→title, question→aiQuestion, answer→userAnswer
//                 ts/timestamp/created/created_at/date → createdAt
//                 subTitle→subtitle, tag→tags, mood/moodLabel→tags[mood:*], moodScore→tags[moodScore:*]
//                 source/sessionId→sourceRef, hidden/isHidden/softHidden→hidden
// • UI-Hilfen: computedTitle, previewText(), badge (Label+Icon)
// • Zeit: createdAt wird intern UTC gehalten; createdAtLocal liefert lokale Zeit
...
class JournalEntry {
  ...
  /// - Journal: thoughtText
  /// - Story: storyTeaser (oder storyTitle als Fallback)
  String get text {
    switch (kind) {
      case EntryKind.reflection:
  ...
  String get computedTitle {
    ...
    switch (kind) {
      case EntryKind.journal:
        if (_isNotEmpty(thoughtText)) return _firstWords(thoughtText!, 6, prefix: 'Tagebuch: ');
        return 'Tagebuch';
      case EntryKind.reflection:
        if (_isNotEmpty(userAnswer))  return _firstWords(userAnswer!, 8,  prefix: 'Thema: ');
        if (_isNotEmpty(aiQuestion))  return _firstWords(aiQuestion!, 6,  prefix: 'Frage: ');
        return 'Reflexion';
      case EntryKind.story:
  ...
  static JournalEntry fromMap(Map<String, dynamic> map) {
    ...
    // ---- Kind (explizit: kind/type; bool: isReflection / isStory / isJournal; sonst Heuristik)
    EntryKind kind;
    ...
    final thoughtText = _asString(map['thoughtText']) ?? _asString(map['text']) ?? _asString(map['body']);
    final aiQuestion  = _asString(map['aiQuestion']) ?? _asString(map['question']) ?? _asString(map['prompt']);
    final userAnswer  = _asString(map['userAnswer']) ?? _asString(map['answer']) ?? _asString(map['response']);
