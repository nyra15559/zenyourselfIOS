...
// v11-ready (2025-09-26) – kompatibel zu Worker v11.4.0
// -----------------------------------------------------------------------------
// Endpoints (Cloudflare Worker):
//   GET  /health
//   POST /reflect               → Spiegel + 1 Leitfrage
//   POST /reflect_full          → Legacy-Schema (Proxy/Alias)
//   POST /next_turn_full        → v11 Alias (Fortsetzung)
//   POST /closure_full          → v11 Abschluss (Mood-Intro, Risiko-Flag)
//   POST /story
//   POST /journey
//   POST /mood
//
// Highlights:
// - v11-Shims: startSessionFull()/nextTurnFull()/closureFull() nutzen neue Endpunkte,
//   mit sauberem Fallback auf /reflect_full bzw. /reflect.
// - Exakter Offline-/Fehler-Text gemäß Projekt-Policy:
//     "Send Yourself hat die Blümchen nicht gefunden. Bitte Verbindung prüfen."
//   (Nur bei echten Verbindungs-/Workerfehlern anzeigen!)
// - toJson(): schreibt KEIN 'output_text', wenn er exakt dem Fehlertext entspricht.
// - Robustes Parsing (primary/question/choices/text/plain, alt keys)
// - PII-Redaktion (E-Mail, Tel, URL, IBAN, Card) – sanft & stabil.
// -----------------------------------------------------------------------------
...
// ------------------------------------------------------------
// Typalias für injizierbaren HTTP-Invoker (siehe ApiService.configureHttp)
// ------------------------------------------------------------
...
class ApiService {
  ...
  /// Convenience: Direkt per Worker-Basisurl + optionalem Token verdrahten.
  ...
  static String _normalizeBase(String base) {
    var b = base.trim();
    // trailing Slashes entfernen (einheitlich ohne), Protokoll bleibt unangetastet
  ...
  /// v11 bevorzugt: voller Contract
  Future<ReflectionTurn> startSessionFull({
    required String text,
    ReflectionSession? session, // tolerant
    String locale = 'de',
    String tz = 'Europe/Zurich',
    int maxTurns = 3,
    List<Map<String, String>>? history,
  ...
  Future<ReflectionTurn> nextTurnFull({
    ...
    // Erst /next_turn_full (v11), dann Fallback /reflect_full → /reflect
  ...
  /// Alias: Fortsetzung gegen /reflect (nicht-full)
  ...
  Future<ReflectionTurn> _reflectStep({
    ...
    // Sanfter Backoff mit leichtem Jitter
    const bases = [420, 900, 1800];
    for (int i = 0; i < bases.length; i++) {
      try {
        final json = await _http!('/reflect', payload).timeout(_timeout);
  ...
  /// Voller Contract gegen /reflect_full (v11-kompatibel)
  ...
  // =======================================================================
  //  analyze() → /reflect_full (tieferes JSON, robust)
  // =======================================================================
  ...
  Future<ReflectionAIResult> aiReflect({
    ...
    final turn = await _reflectStep(
      ...
      session: ReflectionSession(
          threadId: _uuid.v4(), turnIndex: 0, maxTurns: 3),
      locale: 'de',
      tz: 'Europe/Zurich',
      history: messages,
    );
      ...
      for (int i = 0; i < bases.length; i++) {
        try {
          ...
          if (body.trim().isEmpty) {
            throw Exception('Leere Story erhalten');
          }
          ...
          if (i < bases.length - 1) {
            ...
            await Future.delayed(
                Duration(milliseconds: bases[i] + jitter));
            continue;
          }
        }
      }
  ...
  Future<JourneyInsights> journey({
    ...
    if (_http != null && useServerIfAvailable) {
      try {
        ...
        final json =
            await _http!('/journey', payload).timeout(_timeout);
  ...
  // =======================================================================
  Future<StructuredThoughtResult> structureThoughts(
    String input, {
    bool useServerIfAvailable = true,
  }) async {
  ...
  String _coreIdeaFrom(String fullText, List<String> bullets, String lever) {
    ...
    int scoreOf(String s) {
      ...
      if (lever == 'Gedanken' &&
          _any(t, const ['sollte', 'muss', 'immer', 'nie'])) score += 2;
      if (lever == 'Gefühle' &&
          _any(t, const ['fühl', 'angst', 'wut', 'traur'])) score += 2;
      if (lever == 'Körper' &&
          _any(t, const ['körper', 'müde', 'schlaf', 'atem'])) score += 2;
      if (lever == 'Verhalten' &&
          _any(t, const ['aufschieb', 'start', 'beginnen', 'klein'])) score += 2;
      if (lever == 'Kontext' &&
          _any(t, const ['arbeit', 'termin', 'chef', 'famil'])) score += 2;
      return score;
    }
  ...
  // =======================================================================
  //  v11 Abschluss: closureFull
  // =======================================================================

  /// Schickt eine Abschluss-/Mood-Einleitungs-Anfrage an den Worker (v11).
  /// Erwartet JSON mit { closure?: String, mood_intro?: String, risk_level?: "high"|"mild"|"none" }.
  Future<Map<String, dynamic>> closureFull({
    ...
    if (_http == null) return <String, dynamic>{'closure': '', 'risk': false};

    final sessionMap = session is ReflectionSession
        ? session.toJson()
        : (session is Map ? session : {'id': session.toString()});
    ...
    try {
      ...
      try {
        json = await _http!('/closure_full', payload).timeout(_timeout);
      } catch (_) {
        // Fallbacks
        try {
          json = await _http!('/reflect_full', payload).timeout(_timeout);
        } catch (_) {
          json = await _http!('/reflect', payload).timeout(_timeout);
        }
      }

      final closure =
          (json['closure'] ?? json['mood_intro'] ?? json['text'] ?? '')
              .toString();
      final riskLevel =
          (json['risk_level'] ?? json['risk'] ?? 'none').toString().toLowerCase();
      final risk = (riskLevel == 'high' || riskLevel == 'crisis');
      return <String, dynamic>{
        'closure': closure,
        'risk': risk,
        'risk_level': riskLevel,
        'session': json['session'] ?? sessionMap,
      };
    } catch (_) {
      return <String, dynamic>{'closure': '', 'risk': false};
    }
  }
  ...
  ReflectionTurn _turnFromReflectJson(
    ...
    final questionsList =
        _parseStringList(json['questions'] ?? json['multi_questions'] ?? json['qs']);
    ...
    // 2) Primary/Output (für evtl. Anzeige/Legacy)
    ...
    // 3) Separate "erste" Frage (für Screen.question)
    ...
    final schoolsDyn = (json['schools'] as List?) ??
        ...
        const [];
    final normalizedSchools =
        _normalizeSchools(_parseStringList(schoolsDyn));
...
class GuidanceService {
  ...
  void configureHttp(
      ...
      ApiService.instance
          .configureHttp(invoker: invoker, baseUrl: baseUrl, timeout: timeout);

  void configureForWorker(
          {required String baseUrl, String? appToken, Duration? timeout}) =>
  ...
  /// v11 Full-Contract – vom Screen direkt verwendet.
  Future<ReflectionTurn> startSessionFull({
    required String text,
    ReflectionSession? session, // tolerant
    String locale = 'de',
    ...
    List<Map<String, String>>? history,
      ...
      ApiService.instance.startSessionFull(
        text: text,
        session: session,
        locale: locale,
        tz: tz,
  ...
  Future<ReflectionTurn> nextTurnFull({
    ...
    List<Map<String, String>>? history,
      ...
      ApiService.instance.nextTurnFull(
        session: session,
        text: text,
        locale: locale,
        tz: tz,
  ...
  /// Alias: Fortsetzung gegen /reflect
  Future<ReflectionTurn> nextTurn({
    required ReflectionSession session,
    required String text,
    String locale = 'de',
  ...
  Future<StructuredThoughtResult> structureThoughts(String input,
          {bool useServerIfAvailable = true}) =>
      ApiService.instance
          .structureThoughts(input, useServerIfAvailable: useServerIfAvailable);

  Future<Question> fetchGuidingQuestion(
          {String? contextText, bool followUp = false}) =>
      ApiService.instance
          .fetchGuidingQuestion(contextText: contextText, followUp: followUp);
  ...
  Future<String?> detectEmotion(String text) =>
      ApiService.instance.detectEmotion(text);
  Future<int?> classifyMood(String text) =>
      ApiService.instance.classifyMood(text);
  Future<List<String>> suggestTags(String text) =>
      ApiService.instance.suggestTags(text);

  /// v11 Abschluss/Mood-Einleitung
  Future<Map<String, dynamic>> closureFull({
    required dynamic session,
    required String answer,
    String locale = 'de',
    String tz = 'Europe/Zurich',
      ...
      ApiService.instance.closureFull(
        session: session,
        answer: answer,
        locale: locale,
        tz: tz,
  ...
  /// Alias für Kompatibilität: gleich wie `closureFull(...)`
  Future<Map<String, dynamic>> closure({
    required dynamic session,
    required String answer,
    String locale = 'de',
    String tz = 'Europe/Zurich',
...
class ReflectionTurn {
  ...
  final ReflectionFlow? flow; // recommend_end, suggest_break, risk_notice, talk_only
  final ReflectionSession session; // id/turn/max_turns
  final List<String> tags; // inkl. Schulen/Linsen
  final String riskFlag; // none | support | crisis
  final List<String> questions; // alle gelieferten Fragen (erste = Leitfrage)
  ...
  /// Erste sinnvolle Leitfrage (für `question` in toJson)
  String? get primaryQuestion {
    ...
    if (t.isNotEmpty && t.endsWith('?') && t != ApiService.errorHint) return t;
    return null;
  }

  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{
      if (outputText.trim() != ApiService.errorHint) 'output_text': outputText,
...
class StoryResult {
  ...
  const StoryResult(
      {required this.id, required this.title, required this.body, this.audioUrl});
}
