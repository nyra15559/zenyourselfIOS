...
extension ToneTypeWire on ToneType {
  String get wire {
    switch (this) {
      ...
      case ToneType.hope:
        return 'hope';
      case ToneType.gentle:
        return 'gentle';
      case ToneType.insight:
        return 'insight';
      case ToneType.ritual:
        return 'ritual';
      case ToneType.light:
        return 'light';
      case ToneType.crisis:
        return 'crisis';
      case ToneType.unknown:
        return 'unknown';
    }
  }
}
...
ToneType parseToneType(dynamic v) {
  ...
  switch (s) {
    ...
    case 'hope':
      return ToneType.hope;
    case 'gentle':
      return ToneType.gentle;
    case 'insight':
      return ToneType.insight;
    case 'ritual':
      return ToneType.ritual;
    case 'light':
      return ToneType.light;
    case 'crisis':
      return ToneType.crisis;
    default:
...
class Facets {
  ...
  static Facets? fromMaybe(dynamic v) {
    ...
    if (v is Map) {
      ...
      return Facets(
        active: (a?.trim().isEmpty ?? true) ? null : a!.trim(),
        queue: q,
      );
    }
    return null;
...
class SkillBlock {
  ...
  static SkillBlock? fromMaybe(dynamic v) {
    ...
    final stage = parseStage(m['stage']);
    final listDyn = (m['cards'] is List)
        ? (m['cards'] as List)
        : (m['card'] is Map ? [m['card']] : const []);
...
class UserAction {
  ...
  static UserAction? fromMaybe(dynamic v) {
    ...
    final enumType =
        parseActionType(m['action_type'] ?? m['type'] ?? m['action']);
...
class ReflectionTurn {
  ...
  // v7.1.3: understanding.tags separat verfügbar
  ...
  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{
      ...
      if (skillCards.isNotEmpty)
        'skill_cards': skillCards.map((c) => c.toJson()).toList(),
      ...
      // S4.1
      if ((smalltalkReply ?? '').trim().isNotEmpty)
        'smalltalk_reply': smalltalkReply!.trim(),
    };
  ...
  static ReflectionTurn? fromMaybe(dynamic v) {
    ...
    final tsDedup = _orderedDedup(<String>[
      ...topLevelTopicSugs,
    ...
    final speechMeta =
        SpeechMeta.fromMaybe(m['speech_meta'] ?? m['speechMeta']);
    final skill =
        SkillBlock.fromMaybe(m['skill'] ?? m['skill_block'] ?? m['skillBlock']);
    ...
    final facets =
        Facets.fromMaybe(m['facets'] ?? m['facet_queue'] ?? m['facetQueue']);
    final topicPin =
        (m['topic_pin'] ?? m['topicPin'])?.toString().trim();
    final availableActions =
        _orderedDedup(_strings([m['available_actions'], m['actions']]));
...
class ReflectionFlow {
  ...
  static ReflectionFlow? fromMaybe(dynamic v) {
    ...
    return ReflectionFlow(
      ...
      moodPrompt:
          m['mood_prompt'] == true || m['moodPrompt'] == true || moodPromptNested(),
...
class ReflectionSession {
  ...
  /// Backcompat für ältere Callsites, die `session.id` erwarten.
  ...
  static ReflectionSession? fromMaybe(dynamic v) {
    ...
    final id =
        (m['id'] ?? m['thread_id'] ?? m['threadId'] ?? '').toString();
    final turn =
        _intOf(m['turn'] ?? m['turn_index'] ?? m['turnIndex'], 0);
    final max =
        _intOf(m['max_turns'] ?? m['maxTurns'], 3);
...
// ────────────────────────────────────────────────────────────────────────────
// TimelineMarker (Mini-DTO) — v7.1.4+timeline.2 (2025-11-10)
// ────────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────────
// InsightFact (Mini-DTO) — v7.1.4+insight.3 (2025-11-10)
// ────────────────────────────────────────────────────────────────────────────
...
class InsightFact {
  ...
  static InsightFact? fromMaybe(dynamic v) {
    ...
    String _isoDate(dynamic x) {
      final raw = (x ?? '').toString().trim();
      if (raw.isEmpty) return '';
      // Falls ISO-Datetime → auf YYYY-MM-DD gekürzt
      if (raw.length >= 10 &&
          RegExp(r'^\d{4}-\d{2}-\d{2}').hasMatch(raw)) {
        return raw.substring(0, 10);
      }
    ...
    final fact =
        _nz(m['fact'] ?? m['value'] ?? m['text'] ?? m['statement']) ?? '';
    final since = _nz(
      _isoDate(
        m['since'] ??
            m['date'] ??
